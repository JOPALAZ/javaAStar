...
class smartQueue
{
    ...
    public boolean addElemet(Node candidate)
    {
        ...
        for(Node resident : queue)
        {
            ...
            {
                trigered = true;
                if(resident.getFCost() == candidate.getFCost() && resident.getHCost() > candidate.getHCost() || resident.getFCost() > candidate.getFCost())
                {
...
class Node
{
    ...
    public Node(Coordinate father,Integer cost,FieldStates state,Coordinate pos)
    {
        this.father=father;
        this.GCost=cost;
        this.HCost=cost;
    ...
    public void ChangeState(FieldStates state)
    {
        this.state=state;
    }
    public void ChangeFather(Coordinate father)
    {
        this.father=father;
    }
...
public class AStar {
    ...
    private Coordinate makeAnOperation()
    {
        ...
        field[candidate.getPos().x][candidate.getPos().y].ChangeCosts(candidate.getGCost(), candidate.getHCost());
        field[candidate.getPos().x][candidate.getPos().y].ChangeCosts(candidate.getGCost(), candidate.getHCost());
        field[candidate.getPos().x][candidate.getPos().y].ChangeFather(candidate.getFather());
        if(field[candidate.getPos().x][candidate.getPos().y].getState() != FieldStates.Finish)
        {
            field[candidate.getPos().x][candidate.getPos().y].ChangeState(FieldStates.Discovered);
        }
    ...
    {
        return (Math.abs(finish.x-point.x)+Math.abs(finish.y-point.y))*actionCost;
    }
    static public void test()
    {
        ...
        for(int i=0;i<100;++i)
        {
            queue.addElemet(new Node(null,i,FieldStates.Empty,new Coordinate(i,i)));
        }
        for(int i=0;i<100;++i)
        {
            System.out.println(queue.peekLowest().getFCost() + " " + queue.pollLowest().getHCost());
        }
    }
    ...
    private void getPossibilities(Coordinate pos)
    {
        ...
        for(i=(pos.x-1);i<pos.x+2;++i)  
        {
            ...
            {
                if(i>=0&&i<sizeX&&j>=0&&j<sizeY&&field[i][j].getState()!=FieldStates.Wall)
                {
                    if(i!=pos.x||j!=pos.y)
                    {
                        candidate = field[i][j].getPos();
                        GCost=(int)(Math.sqrt(Math.abs(i-pos.x)+Math.abs(j-pos.y))*actionCost) + field[pos.x][pos.y].getGCost();
                        HCost=calculateHypothetical(candidate);
                        if(!searched.contains(candidate))
                        {
                            candidateNode = new Node(pos,GCost+HCost, FieldStates.Known,candidate);
